...
export type MapPoint = { evaluation_id:number;path_id:number;outcome_state:string;pass_a:boolean;pass_b:boolean;q_rel:number;q_abs:number;x:number;y:number };
export type MapData = { study_id:number;snapshot_id:number;axis_x:string;axis_y:string;target:string;color:string;cells:Cell[];points:MapPoint[] };
