...
import { Mail, LockKeyhole, type LucideIcon } from "lucide-react";
...
export function LoginPage() {
  ...
  return (
    <form className="space-y-4" onSubmit={onSubmit}>
      <div className="mb-6">
        <h2 className="text-center text-xl font-semibold text-slate-100">{t("auth.loginTitle")}</h2>
      </div>
      ...
      <Link className="block text-center text-sm text-[#2dd4bf] transition hover:text-[#99f6e4]" to="/register">
        {t("auth.noAccount")}
      </Link>
    </form>
  );
