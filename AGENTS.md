...
本專案以 `docs/` 三份真源文件作為既有穩定系統的最高需求來源。新功能、重大更新與研究草案必須另建獨立文件，不得混入三份真源文件。所有代理工作需遵守 `CLAUDE.md`，並保持輸出為正體中文。
...
9. 新功能、重大更新、研究草案、MVP 範圍與未確認規格一律另建或更新獨立 `docs/` 文件，不得寫入三份真源文件。三份真源文件只保留既有穩定系統規格，不得混入本輪討論稿或 Agent 自行推導的內容。
...
- `docs/多資產與宏觀指標研究升級計畫.md` 是本輪討論稿、草案收容處與開發指導，不直接取代三份真源文件；外部指標與多資產規格在使用者確認前必須停留於獨立文件，不得寫回三份真源文件。
