...
export function DynamicParametersPage() {
  ...
  const request: CreateDynamicStudy = {
    ...
    long_term_filter_enabled: true, long_term_filter_months: 10, confirm_soft_limit: confirmSoftLimit
  };
  ...
  const previewMutation = useMutation({ mutationFn: (input: CreateDynamicStudy) => dynamicParametersApi.preview(input), onSuccess: (value, input) => { setPreviewSignature(JSON.stringify(input)); setConfirmSoftLimit(!value.requires_confirmation); } });
  ...
  const materializeMutation = useMutation({ mutationFn: () => dynamicParametersApi.materialize(selectedStudyId, false), onSuccess: (value) => { setTaskId(value.task.id); queryClient.setQueryData(["dynamic-parameter-study", selectedStudyId], value.study); } });
  ...
  return <section className="space-y-5">
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1.2fr)_minmax(320px,.8fr)]">
      <Card><CardHeader><div><CardTitle>建立模型研究</CardTitle><CardDescription>兩條模型路線不會自動互相比較；請先選路線，再用同一路線內的 one-standard-error 規則選擇回看期。</CardDescription></div></CardHeader>
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          ...
          <Field label="Beta 控制模式"><select className={inputClass} value={mode} onChange={(event) => setMode(event.target.value as ParameterMode)}><option value="fixed">固定</option><option value="global">全域</option><option value="continuous">連續動態</option><option value="six_state">六狀態切換</option></select></Field>
          ...
          <Field label="Walk-forward 折數"><input className={inputClass} type="number" min={2} max={10} value={folds} onChange={(event) => setFolds(Number(event.target.value))} /></Field>
          ...
          <Field label="最少訓練筆數"><input className={inputClass} type="number" min={20} value={minimumTrain} onChange={(event) => setMinimumTrain(Number(event.target.value))} /></Field>
          <div className="md:col-span-2 xl:col-span-3"><Field label="候選回看期"><input className={inputClass} value={lookbacksText} onChange={(event) => setLookbacksText(event.target.value)} /><span className="text-xs text-slate-500">可用：5、10、20、40、60、120、250、500</span></Field></div>
        </div>
        <div className="mt-5 flex flex-wrap gap-2"><Button variant="secondary" disabled={!formValid || previewMutation.isPending} onClick={() => previewMutation.mutate(request)}><RefreshCw className="mr-2 h-4 w-4" />預覽計算</Button><Button disabled={!formValid || previewSignature !== requestSignature || createMutation.isPending} onClick={() => createMutation.mutate()}><Workflow className="mr-2 h-4 w-4" />建立訓練任務</Button></div>
        ...
        {previewMutation.data && <div className="mt-5"><ComputePlanSummary preview={previewMutation.data} /></div>}
      </Card>
    ...
    {study?.artifact_set_hash && <Card><CardHeader><div><CardTitle>模型、校準與追溯</CardTitle><CardDescription>模型、OOF 預測、政策與報告均固定到內容雜湊；未通過 baseline gate 的目標不得供動態持倉使用。</CardDescription></div>{study.status === "completed" && <CheckCircle2 className="h-5 w-5 text-teal-300" />}</CardHeader>
      <div className="grid gap-3 md:grid-cols-3"><Hash label="模型組" value={study.artifact_set_hash} /><Hash label="研究設定" value={study.setting_hash} /><Hash label="資料集" value={study.dataset_hash} /></div>
      <div className="mt-4 overflow-x-auto"><table className="w-full text-left text-sm"><thead className="text-xs text-slate-500"><tr><th className="p-2">預測期</th><th className="p-2">目標</th><th className="p-2">模型 loss</th><th className="p-2">Baseline</th><th className="p-2">Gate</th></tr></thead><tbody>{(study.reports ?? []).map((report, index) => <tr key={`${report.horizon}-${report.target_kind}-${index}`} className="border-t border-white/[0.05]"><td className="p-2 text-slate-300">{report.horizon} 日</td><td className="p-2 text-slate-300">{report.target_kind}</td><td className="p-2 text-slate-400">{report.mean_loss?.toFixed(5)}</td><td className="p-2 text-slate-400">{report.mean_baseline_loss?.toFixed(5)}</td><td className={`p-2 ${report.baseline_gate_passed ? "text-teal-300" : "text-amber-300"}`}>{report.baseline_gate_passed ? "通過" : "尚未證明"}</td></tr>)}</tbody></table></div>
      {study.status === "awaiting_materialization" && <div className="mt-4"><Button onClick={() => materializeMutation.mutate()} disabled={materializeMutation.isPending}><Workflow className="mr-2 h-4 w-4" />建立每日物化與回測任務</Button></div>}
      {study.comparison && <div className="mt-5 space-y-3"><div className="flex flex-wrap gap-2">{study.comparison.available_blocks.map((id) => <Button key={id} variant={blockId === id ? "primary" : "secondary"} disabled={id === "daily-diagnostics" && !study.materialization_id} onClick={() => setBlockId(id)}>{id}</Button>)}</div>{blockQuery.data && <pre className="max-h-96 overflow-auto rounded-lg border border-white/[0.06] bg-slate-950/70 p-3 text-xs text-slate-300">{JSON.stringify(blockQuery.data.payload, null, 2)}</pre>}</div>}
    </Card>}
  </section>;
}
